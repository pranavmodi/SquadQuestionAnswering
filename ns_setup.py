...
def process_file(filename, tokenizer, batch_size=64, cls='[CLS]',
                 sep='[SEP]', output_dir='bertembedding'):
        ...
        for article in tqdm(source["data"]):
            ...
            for para in tqdm(article["paragraphs"]):
                ...
                for qas in para['qas']:
                    ...
                    qas_text = ' '.join([cls, context, sep, qas['question']])
                    qas_tokens = tokenizer.tokenize(qas_text)
                    indices.append(tokenizer.encode(qas_tokens))
